...
void twoPartyTripleGen(u32 numParties, u32 myIdx, u32 idx, u32 logNum, u32 numThreads, Socket &chl, std::string fileName)
{
    ...
    coproto::sync_wait(gmw.generateTriple(1 << 28, numThreads, chl));
    // write A, B, C, D
...
void tripleGenParty(u32 idx, u32 numParties, u32 logNum, u32 numThreads){
    Timer timer;
    ...
    if(idx == 0){
        ...
        auto time = std::chrono::duration_cast<std::chrono::microseconds>(iter->first - timer.mTimes.front().first).count() / 1000.0;
            	std::cout<< " " << std::endl;
            	std::cout<< "P1 offline pre time cost = " << std::fixed << std::setprecision(3) << time/1000.0 << " s"  << std::endl;
            	std::cout << "P1 offline pre communication cost = "<< std::fixed << std::setprecision(3)  << comm / 1024 / 1024 << " MB" << std::endl;
            	std::cout<< " " << std::endl;    
    }
